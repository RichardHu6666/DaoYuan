...
function updateIdentityState() {
  ...
  const sourceText = identitySource === "qq" ? "QQ 链接进入" : identitySource === "account" ? "账号登录" : "身份已识别";
  ...
  if (refs.chatIdentityCard) {
    refs.chatIdentityCard.textContent = identitySource === "qq" ? "QQ 免登录" : currentAccount ? `账号 ${currentAccount}` : "未登录";
  }
}
...
function renderChatHistory(messages = []) {
  ...
  if (!visibleMessages.length) {
    appendChatMessage(
      "assistant",
      "这里是 Campus Jarvis 的网页聊天入口。它和 QQ bot 共享用户资料，但网页会话上下文单独保存。你可以问：我是谁、今天有什么DDL、有什么推送、我的课表。"
    );
    return;
  }
...
async function sendChatMessage(overrideMessage = "") {
  const message = (overrideMessage || refs.chatInput.value).trim();
