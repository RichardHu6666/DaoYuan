...
class LLMValidateService:
    ...
    def validate(self, temp_event: TempEvent, original_body: OriginalBody, rule_result: dict) -> dict:
        if self.client.is_available():
            return self._validate_remote(temp_event, original_body, rule_result)
        return self._validate_fallback(temp_event, original_body, rule_result)

    def _validate_remote(self, temp_event: TempEvent, original_body: OriginalBody, rule_result: dict) -> dict:
        system_prompt = (
        ...
        return {
            ...
            "source_type": _source_type_from_page_id(temp_event.page_id),
        }

    def _validate_fallback(self, temp_event: TempEvent, original_body: OriginalBody, rule_result: dict) -> dict:
        source_type = _source_type_from_page_id(temp_event.page_id)
        ...
        if source_type != "wechat_db" and temp_event.title.strip() and temp_event.title.strip() not in original_body.original_text:
            return {
                "semantic_passed": False,
...
def _source_type_from_page_id(page_id: str) -> str:
